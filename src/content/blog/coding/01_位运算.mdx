...
import LinkPreview from '@/custom/components/advanced/LinkPreview.astro'
import TabItem from '@/custom/components/user/TabItem.astro'
import Tabs from '@/custom/components/user/Tabs.astro'
import Collapse from '@/custom/components/user/Collapse.astro'
...
<Aside type="note" title="并行计算置位数 by Gemini-2.5-Pro" open="false" style="card">
...
<Aside type="note" icon="deep_seek"  open="false" style="card">
<span slot="title">为什么 `log2WithDeBruijn(v)` 等价于 `31 - Integer.numberOfLeadingZeros(v)`?</span>
#### **1. 方法目标**
...
import Steps from '@/custom/components/user/Steps.astro'
import Timeline from '@/custom/components/user/Timeline.astro'
